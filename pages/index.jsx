...
function __buildTests() {
  ...
  // Cascade delete size
  results.push(["Cascade delete from A has 3 ids", new Set(getDescendantIds("A", list)).size === 3]);
...
export default function Page() {
  ...
  // Auto defaults for parent options on add form
  useEffect(() => {
    if (type === "Subsystem") {
  ...
  function addNode() {
    try {
      ...
      if (dupe) {
        ...
        if (overwrite) {
          pushHistory();
          setNodes((prev) => prev.map((x) => (x.id === dupe.id ? { ...x, name: nm, type, parentId: parent ? parent.id : null, level: type === "Subsystem" ? lvl : null, createdAt: safeNow() } : x)));
      ...
      pushHistory();
      const newNode = { id: uid(), name: key.name || nm, type, parentId: parent ? parent.id : null, level: type === "Subsystem" ? lvl : null, createdAt: safeNow() };
